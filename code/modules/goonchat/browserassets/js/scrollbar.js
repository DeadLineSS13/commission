var ssb = {
	...
	scrollbar : function (cont_id) {
	  ...
	  cont.st = this.create_div('scrollbar_track', cont, cont_clone);
	  cont.sb = this.create_div('scrollbar_handle', cont, cont_clone);
	  cont.su = this.create_div('scrollbtnup', cont, cont_clone);
	  cont.sd = this.create_div('scrollbtndown', cont, cont_clone);
	  ...
	  cont.sb.onmouseover = function (e) {
		if (! this.cont.sg) this.className = 'scrollbar_handle ssb_sb_over';
		return false;
	  }
	  
	  // on mouse out - revert back our usual class name 'scrollbar_handle'
	  cont.sb.onmouseout  = function (e) {
		if (! this.cont.sg) this.className = 'scrollbar_handle';
		return false;
	  }
	...
	mousedown : function (o, s) {
	  if (ssb.sc == 0) {
		// new class name
		o.cont.sb.className = 'scrollbar_handle';
		ssb.asd = o.cont;
		ssb.sc = s;
		ssb.sp = 400;
		ssb.arrow_scroll();
	...
	onmouseup : function (e) {
	  ...
	  // new class name
	  if (ssb.asd) ssb.asd.sb.className = (tg.className.indexOf('scrollbar') > 0) ? 'scrollbar_handle ssb_sb_over' : 'scrollbar_handle';
...
window.onload = function() {
	ssb.scrollbar('scrollbar_content'); // scrollbar initialization
}
