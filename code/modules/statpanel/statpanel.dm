...
/client/proc/add_verb(path)
	verbs |= path
	mob?.updateStatPanel()


/client/proc/remove_verb(path)
	verbs -= path
	mob?.updateStatPanel()

/mob/proc/add_verb(path)
	verbs |= path
	updateStatPanel()

/mob/proc/remove_verb(path)
	verbs -= path
	updateStatPanel()
...
/client/proc/lobbyPig()
	src << browse('code/modules/statpanel/html/html/statpanel.html', "window=outputwindow.browser;")
